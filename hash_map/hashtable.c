...
size_t hash_base(char const* s, size_t table_size, unsigned base) {
  size_t ret = 0;
  for (int i = 0; s[i]; i++) {
    // i use 1ULL to cast ret to unsigned long long before multiplication to avoid overflow
    ret = (size_t)((1ULL * ret * (unsigned long long)base + (unsigned char)s[i]) % (unsigned long long)table_size);
  }
  return ret;
}
...
void hashtable_delete(hashtable* h) {
  for (size_t i = 0; i < h->size; ++i)
    if (h->key[i]) free((void*)h->key[i]);
  free(h->key);
  for (size_t i = 0; i < h->size; ++i)
    if (h->value[i]) free((void*)h->value[i]);
  free(h->value);
  free(h);
}
...
size_t hashtable_position(hashtable* h, char const* key) {
  for (size_t f = 0; f < alter; ++f) {
    ...
    // if the slot is empty, it's a new entry
    if (h->key[pos] == 0) {
      h->key[pos] = duplicate(key);
      return pos;
    }
    // if the slot already contains the key, return its position
    if (strcmp(h->key[pos], key) == 0) {
      return pos;
    }
    // otherwise, try the next hash function in the family
  }
  fprintf(stderr, "no valid hash position found for %s after %zu tries\n", key, alter);
  exit(1);
}
