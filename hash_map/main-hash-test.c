...
unsigned hash_simple(char const* s, size_t table_size) {
  if (!s || !s[0]) return 0;
  unsigned sum = (unsigned char)s[0];
  if (s[1]) sum += (unsigned char)s[1];
  return sum % (unsigned)table_size;
}
...
unsigned hash_base(char const* s, size_t table_size) {
  // i use unsigned long long to minimize early overflow before the final modulo operation
  unsigned long long res = 0;
  for (int i = 0; s[i]; i++) {
    res = res * base + (unsigned char)s[i];
  }
  return (unsigned)(res % table_size);
}
...
unsigned hash_base_mod(char const* s, size_t table_size) {
  unsigned res = 0;
  for (int i = 0; s[i]; i++) {
    // i use 1ULL to cast res to unsigned long long before multiplication to avoid overflow
    res = (unsigned)((1ULL * res * base + (unsigned char)s[i]) % table_size);
  }
  return res;
}


#define DICO "hashmap-students/dsa2-hashmap/students/dico-francais.txt"
...
int main(int argc, char* argv[argc]) {
  ...
  size_t *tableS = calloc(m, sizeof(size_t));
  size_t *table = calloc(m, sizeof(size_t));
  size_t *tableB = calloc(m, sizeof(size_t));
  size_t *tableM = calloc(m, sizeof(size_t));
